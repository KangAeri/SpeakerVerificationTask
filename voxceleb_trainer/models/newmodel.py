...
class MainModel(nn.Module):
    def __init__(self, in_channels=80, channels=512, nOut=192,**kwargs):
        super().__init__()
        self.convert = torchaudio.transforms.MFCC(
            sample_rate = 16000,
            n_mfcc=in_channels,
        ...
        self.linear = nn.Linear(3072, nOut)
        self.bn2 = nn.BatchNorm1d(nOut)

    def forward(self, x:Tensor):
        #import pdb;pdb.set_trace()
        x = self.convert(x) # (B, L) -> (B, 80, L`)
        #x = x.transpose(1, 2) # (B, 80, L`) -> (B, L`, 80)
        out1 = self.layer1(x) 
